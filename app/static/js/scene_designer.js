  ...
  function renderInspector() {
    ...
    if (node.node_type === "VLM") {
      ...
      fields = node.node_type === "START"
        ? `<div class="system-node-note"><strong>开始节点</strong><p>系统始终提供 <code>{{ input.image_path }}</code>。在下方新增键和值后，配方 ROI 才能绑定对应的校验值。</p></div>${parameterSectionMarkup({ id: "nodeStartInputParameters", title: "流程输入参数", description: "参数名称只用于页面展示，参数键用于配方绑定和节点引用。", rows: schemaRows(config.inputs || []), kind: "SCHEMA", readonly, keyLabel: "参数键", valueLabel: "参数名称", keyPlaceholder: "例如 ocr_text", valuePlaceholder: "例如 OCR 校验文字", variableScope: "NONE" })}`
